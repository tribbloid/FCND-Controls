...
class ControllerReference(object):
    ...
    def roll_pitch_controller(self, acceleration_cmd, attitude, thrust_cmd):
        ...
        if thrust_cmd > 0.0:
            target_R13 = -np.clip(acceleration_cmd[0].item() / c_d, -self.max_tilt,
                                  self.max_tilt)  # -min(max(acceleration_cmd[0].item()/c_d, -self.max_tilt), self.max_tilt)
            target_R23 = -np.clip(acceleration_cmd[1].item() / c_d, -self.max_tilt,
                                  self.max_tilt)  # -min(max(acceleration_cmd[1].item()/c_d, -self.max_tilt), self.max_tilt)
